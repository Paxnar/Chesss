...
WIDTH, HEIGHT = 1280, 720  # Размеры окна
GAMEMODE = None
...
def greeting_mine_sweeper() -> None:
    """Функция, отображающая стартовую картинку"""
    global GAMEMODE
    global mine_sweeper
    background = pygame.transform.scale(load_image('MineSweeperScreen.png'), (WIDTH, HEIGHT))
...
class MineSweeper:
    ...
    def render_screen(self) -> None:
        ...
        screen.blit(mines_remain, (WIDTH - 140, 484))
    ...
    def lose(self, f_x: int, f_y: int) -> None:
        ...
        with s3.connect('database.db') as con:
            cur = con.cursor()
    ...
    def show_statisctic(self) -> None:
        ...
        Sprite(12, WIDTH - 300, 450, (280, int(280 / 2.12)))
    ...
    def make_sound(s: str) -> None:
        """Проигрывать звуки"""
        soun = pygame.mixer.Sound(os.path.join("sounds", f"{s}"))
